...
| **Worktrees** | Isolated branch per agent so parallel runs don't collide | CodeBuild containers / ECS tasks | One task definition per agent; filesystem-isolated state |
...
| **Sub-agents** | Specialists orchestrated by a coordinator | Bedrock Agents (multi-agent collaboration) | Orchestrator + specialist agents; AgentCore harness |
...
def handler(event, context):
    ...
    response = bedrock_agent.invoke_agent(
        ...
        agentAliasId="PROD",
        sessionId=session_for(event),          # ties back to memory, see below
        inputText=objective,
    )
...
The AWS translation is filesystem-level isolation per agent: a CodeBuild container or an ECS task per agent, one task definition each. Each run gets a clean, isolated environment and tears down after.

I want to be honest about the seam here. The seed maps worktrees to CodeBuild and ECS, and for isolation that holds. But Osmani's worktrees are literally git worktrees inside a running coding agent, and whether you reproduce *that* on Bedrock (agents operating on real git worktrees, not just isolated containers) is a different question I haven't run to ground. <!-- VERIFY: can Bedrock Agents operate on actual git worktrees, or is the CodeBuild/ECS mapping only an isolation analogy? Seeds assert the mapping but don't prove the git-worktree equivalence. --> Treat this row as "isolation, mapped," not "git worktrees, reproduced." I'd rather flag the gap than sell you a clean line that isn't clean.
...
AWS MCP Server reaching GA in May 2026 is what makes this real on AWS, and per Prasad Rao (AWS Principal SA, LinkedIn, May 2026) it's pay-as-you-go with no extra charge for the server itself (<!-- VERIFY: confirm "no extra charge for the Bedrock MCP Server" against current AWS pricing before publishing -->). Practically, MCP is how your orchestrator calls out to external tools without you writing and maintaining glue code for each one.
...
Bedrock Agents supports multi-agent collaboration directly: an orchestrator agent routes work to specialist agents. The AgentCore harness is what holds that arrangement together in production. And "in production" comes with numbers you should know before you design: AgentCore documents a quota of **200 TPS per agent** and **5,000 active sessions per account** ([AgentCore release notes](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/release-notes.html)). Agents hold sessions, sometimes for a long time, so the session ceiling is the one I'd model first. It's easy to blow past 5,000 concurrent sessions with a fleet of long-lived loops and never touch the TPS limit.
...
<!-- VERIFY: confirm exact IAM permissions and ARNs for the EventBridge -> Lambda -> Bedrock Agents chain against current docs before anyone runs this. -->
